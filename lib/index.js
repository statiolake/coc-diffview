...
var Diffview = class {
  ...
  constructor(context) {
    context.subscriptions.push(
      ...
      import_coc.commands.registerCommand("diffview.close", () => this.close()),
      import_coc.commands.registerCommand(
        "diffview.closeDiffAndOpenFile",
        () => this.closeDiffAndOpenFile()
      ),
      import_coc.commands.registerCommand(
        "diffview.toggleLayout",
        () => this.toggleLayout()
      ),
      import_coc.commands.registerCommand(
        "diffview.nextChange",
  ...
  async open(options) {
    ...
    const layout = options.layout ?? import_coc.workspace.getConfiguration("diffview").get("layout", "unified");
  ...
  async closeSession(session, targetBuffer) {
    ...
    if (await windowIsValid(session.returnWindow) && await bufferIsValid(targetBuffer)) {
      await import_coc.workspace.nvim.call("nvim_win_set_buf", [session.returnWindow, targetBuffer]);
      await import_coc.workspace.nvim.call("win_gotoid", [session.returnWindow]);
    }
  ...
  async toggle(options) {
    if (this.session && (!options || sameSource(this.session.options.modified, options.modified))) {
  ...
  async openUnified(options) {
    ...
    const returnBuffer = await import_coc.workspace.nvim.call("nvim_win_get_buf", [editor]);
  ...
  async renderUnified(session) {
    ...
    session.hunkRows = changes.map(
      (change) => Math.min(change.newStart, Math.max(0, current.length - 1))
    );
    for (const change of changes) {
      ...
      if (change.added.length) {
        const highlight = change.removed.length ? "CocDiffviewChanged" : "CocDiffviewAdded";
  ...
  async navigateChange(direction) {
    const session = this.session;
    if (!session) return;
    if (session.kind === "split") {
      await import_coc.workspace.nvim.command(`normal! ${direction > 0 ? "]c" : "[c"}`);
      return;
    }
    const window = await import_coc.workspace.nvim.call("win_getid");
    if (window !== session.window || !session.hunkRows.length) return;
    const cursor = await import_coc.workspace.nvim.call("nvim_win_get_cursor", [window]);
    const currentRow = cursor[0] - 1;
    const target = direction > 0 ? session.hunkRows.find((row) => row > currentRow) : [...session.hunkRows].reverse().find((row) => row < currentRow);
    if (target === void 0) return;
    await import_coc.workspace.nvim.call("nvim_win_set_cursor", [window, [target + 1, 0]]);
  }
  async installNavigationMappings(session, buffers) {
    for (const buffer of new Set(buffers)) {
      const mappings = await import_coc.workspace.nvim.call("nvim_buf_get_keymap", [buffer, "n"]);
      for (const [key, command] of [["]c", "diffview.nextChange"], ["[c", "diffview.previousChange"]]) {
        ...
        await import_coc.workspace.nvim.call("nvim_buf_set_keymap", [buffer, "n", key, `<Cmd>CocCommand ${command}<CR>`, {
          noremap: true,
          silent: true,
          nowait: true
        }]);
  ...
  async removeNavigationMappings(session) {
    for (const buffer of session.mappedBuffers) {
      ...
      for (const [key, command] of [["]c", "diffview.nextChange"], ["[c", "diffview.previousChange"]]) {
        ...
        await import_coc.workspace.nvim.call("nvim_buf_del_keymap", [buffer, "n", key]);
      }
    }
  }
  async defineHighlights() {
    for (const [group, target] of [
      ...
      await import_coc.workspace.nvim.command(`highlight default link ${group} ${target}`);
    }
  }
  async editorWindow() {
...
async function bufferIsFile(buffer) {
  if (!await bufferIsValid(buffer)) return false;
  const type = await import_coc.workspace.nvim.call("nvim_get_option_value", [
    "buftype",
    { buf: buffer }
  ]);
  const name = await import_coc.workspace.nvim.call("nvim_buf_get_name", [buffer]);
